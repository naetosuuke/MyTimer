...
class ViewController: UIViewController {
    ...
    //?をつけているのは、「ラップ処理」つまり、Timerクラスから呼び出した値が空でもエラーをださずに進めて良い
    //(代わりにnilという戻り値を渡す)、という指定をしている
    //これが!だとエラー扱いになり、処理を止められる。こちらはアンラップ処理とよぶ
    //空の状態も想定される定数、変数は?、必ず値が入らなければならない定数、変数は!と書いておけば、予期せぬ動作を事前い阻止できる。
